...
# State machine to avoid flickering
current_emotion = "PLAIN"
last_emotion = "PLAIN"

# Load emoji images
def load_emoji(filename, required=False):
...
try:
    # Load all emoji images - map to actual file names
    emoji_images = {
        "smile": load_emoji("smile.jpg", required=False),
        "laugh": load_emoji("laugh.jpg", required=False),
    ...
    if emoji_images["crazy"] is None:
        emoji_images["crazy"] = load_emoji("wave.jpg", required=False)  # Try wave.jpg for crazy
    if emoji_images["crazy"] is None:
        emoji_images["crazy"] = load_emoji("wink.jpg", required=False)  # Try wink.jpg for crazy
    
    # Set fallbacks between similar emotions
    if emoji_images["smile"] is None:
        emoji_images["smile"] = emoji_images["laugh"]
    if emoji_images["laugh"] is None:
        if emoji_images["smile"] is not None:
            emoji_images["laugh"] = emoji_images["smile"]
        elif emoji_images["surprised"] is not None:
            emoji_images["laugh"] = emoji_images["surprised"]
    if emoji_images["crazy"] is None:
        if emoji_images["surprised"] is not None:
    ...
    image_mapping = {
        "smile": "smile.jpg",
        "laugh": "laugh.jpg → smile.jpg",
        "surprised": "surprised.jpg",
        "sad": "sad.jpg",
        "dance": "dance.jpg",
        "air": "air.jpg",
        "heart": "heart.jpg",
        "crazy": "crazy.jpg → wave.jpg/wink.jpg",
    ...
    print("- smile.jpg, laugh.jpg, surprised.jpg, sad.jpg")
...
print("  6. ❤️  Heart - Form heart with thumbs + index fingers")
...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose, \
    ...
    while cap.isOpened():
        ...
        if results_hands.multi_hand_landmarks and len(results_hands.multi_hand_landmarks) >= 2:
            if detect_heart_gesture(results_hands.multi_hand_landmarks):
                detected_state = "HEART"
